...
from verilogToZ3.Verilog2001Visitor import Verilog2001Visitor
...
class verilogVisitor(Verilog2001Visitor):
	def visitModule_declaration(self, ctx: Verilog2001Parser.Module_declarationContext):
		...
		inp = inp[:-2]
		eq = eq[:-2]
		z3constraint = "B = "+"(And("+eq+"))"
		z3constraint = z3constraint.replace(out, out2)
		z3filecontent = var_dec+"\n"+z3constraint
		formula = "formula = Implies(False, B)"
		z3filecontent += "\n"+formula+"\nvalid(formula)"
